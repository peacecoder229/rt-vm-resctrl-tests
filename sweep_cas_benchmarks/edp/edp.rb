...
require_relative 'SampleInjector'
  ...
  def parse_emon_dat(file,file2,emonMfile,supportFilesFlag)
    ...
    File.readlines(file).each do |line|
	  ...
	  _cpu_index += 1
          ...
          $stderr.puts " ERROR: the emon -M topology seems to be invalid. It may be malformed, or there may be duplicated topology information in the input file."
          exit
        end
      end
    end
    ...
    self.sockets.ids.each do |s_id|
      ...
      loc_sockets = Hash.new
      self.sockets[s_id].each do |key, value|
        loc_sockets[key] = value
      end

      # Remove the unmapped cores, which are cached in loc_sockets, to add in contiguous order
      self.sockets[s_id].clear()
      loc_sockets.keys.each do |k|
        if _phy_to_v_core[s_id][k].nil?
          raise "something went wrong when mapping cores"
        else
          self.sockets[s_id][_phy_to_v_core[s_id][k]] = loc_sockets.delete(k)
        end
      end
    end
...
class EmonData
  ...
  def process_chunk(samples)
    ...
    samples.each do |sample|
      ...
      AdjustPerfmetrics(interval)
      interval.events.each_value do |event|
        loop << event
        aggregated << event
      end
