...
%JRUBY% %JRUBY_OPTIONS% edp.rb -i %EMON_DATA% -f %CHART_FORMAT% -o %OUTPUT% -m %METRICS% -b %BEGIN% -e %END% %VIEW% %TPS% --step 1 -p %PARALLELISM% %OPTIONAL%
%RUBY%  edp.rb -i %EMON_DATA%  -g %CONFIG_FILE% -d %DMIDECODE_FILE% -D %DISKSTAT_FILE% -n %NETWORKSTAT_FILE% -f %CHART_FORMAT% -o %OUTPUT% -m %METRICS% -b %BEGIN% -e %END% %VIEW% %TPS% --step 2 %TIMESTAMP_IN_CHART% %OPTIONAL%
