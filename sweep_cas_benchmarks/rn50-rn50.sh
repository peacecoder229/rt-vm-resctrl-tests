...
do
   ...
   ./hwdrc_icx_2S_xcc_init_to_default_pqos_CAS.sh $casValue 0-$((cores+offset)) 58-59 #$cores-$((cores+offset))
   cd -
   echo "pqos"
   ...
   MLCP1=$!
   docker run --rm --name=rn50_2 --cpuset-cpus=$cores-$((cores+offset)) -e OMP_NUM_THREADS=$cores mxnet_benchmark 2>  rn50_temp_1
   kill -SIGINT $LPMON
   ...
   img_per_sec_2=$(grep "images per second" rn50_temp_2 | awk '{print $3}')
   ...
   echo $cores,$casValue,$img_per_sec_2,$MISSES_rn50_2,$LLC_rn50_2,$MBL_rn50_2 >> ${cores}_rn50_2_monitor.csv
